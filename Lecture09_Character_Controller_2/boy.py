...
def space_down(e): # e가 space key input 인가를 확인 T/F
...
class AutoRun:
    ...
    def draw(self):
        if self.boy.face_dir == 1:
            self.boy.image.clip_draw(self.boy.frame * 100, 100, 100, 100, self.boy.x, self.boy.y+70,300,300)
        else:
            self.boy.image.clip_draw(self.boy.frame * 100, 0, 100, 100, self.boy.x, self.boy.y+70,300,300)
    ...
    def __init__(self):
        ...
        self.state_machine = StateMachine(
            self.IDLE,
            {
                ...
                self.AUTORUN: {a_down: self.IDLE, auto_time_out: self.IDLE, right_down: self.RUN, left_down: self.RUN}
            }
        )
